...
resolve({latitude,longitude})},error=>{kiss.tools.getGeolocationByIP().then(geolocation=>{resolve(geolocation)}).catch(error=>{reject(error)})})}else{kiss.tools.getGeolocationByIP().then(geolocation=>{resolve(geolocation)}).catch(error=>{reject(error)})}})},outlineDOM(state){[].forEach.call($$("*"),function(a){a.style.outline=`${(state) ? "1" : "0"}px solid #`+(~~(Math.random()*(1<<24))).toString(16)})},highlight(element,text){const elementRect=element.getBoundingClientRect()
