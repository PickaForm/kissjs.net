...
if(kiss.theme.currentColor===undefined)kiss.theme.currentColor="light"
kiss.theme.currentGeometry=localStorage.getItem("config-themeGeometry")
if(kiss.theme.currentGeometry===undefined)kiss.theme.currentGeometry="default"
...
document.querySelectorAll("link").forEach(link=>{if(link.href.indexOf("styles/colors")!=-1){if(kiss.global.absolutePath){link.href=kiss.global.absolutePath+"/kissjs/client/ui/styles/colors/"+color+".css"}else{link.href="../../kissjs/client/ui/styles/colors/"+color+".css"}}})}else{let theme=localStorage.getItem("config-theme")
if(!theme)return
theme=JSON.parse(theme)
Object.keys(theme).forEach(variable=>{const color=theme[variable]
document.documentElement.style.setProperty(variable,color)})}}
if(geometry){document.querySelectorAll("link").forEach(link=>{if(link.href.indexOf("styles/geometry")!=-1){if(kiss.global.absolutePath){link.href=kiss.global.absolutePath+"/kissjs/client/ui/styles/geometry/"+geometry+".css"}else{link.href="../../kissjs/client/ui/styles/geometry/"+geometry+".css"}}})}
...
return true},generateSlug(title){let slug=title.toLowerCase().replace(/\s+/g,'-')
slug=slug.replace(/[^a-z0-9-]/g,'').replace(/--+/g,'-')
slug=slug.replace(/^-+|-+$/g,'')
return slug},generateBreadcrumb(items){let breadcrumb='<nav aria-label="breadcrumb"><div itemscope="itemscope" itemtype="http://schema.org/BreadcrumbList" class="breadcrumb"><div class="container">'
